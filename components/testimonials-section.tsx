...
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
const iconMap: { [key: string]: React.ElementType }={ ChevronLeft,
  ChevronRight,
  Star,
  Quote,
  Play,
...
export default function testimonialsSectionCollection({ about }: { about: any }) {
  const [isVisible, setIsVisible] = useState(false)
  const [currentSlide, setCurrentSlide] = useState(0)
  const [isAutoPlaying, setIsAutoPlaying] = useState(true)
  const [isPaused, setIsPaused] = useState(false)
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative">
        ...
        <div className={`relative ${isVisible ? "animate-fade-in-delay" : "opacity-0"}`}>
          ...
          <div className="relative min-h-[500px] top-20 left-48 flex items-center justify-center perspective-1000 px-4">
            <div className="relative w-full max-w-6xl   mx-auto">
              {about.testimonialsSectionCollection.items.map((testimonial:any, index:number) => {
