...
export default function AdvancedGallery({ data }: { data: any }) {
  ...
  const locations = ["All", ...Array.from(new Set(data?.galleryImagesCollection?.items?.map((img:any) => img.location)))]
  ...
  const filteredAndSortedImages = useMemo(() => {
    ...
    // Filter by location
    if (activeFilter !== "All") {
      filtered = filtered.filter((img:any) => img.location === activeFilter)
    }
  ...
  const navigateImage = (direction: "prev" | "next") => {
    ...
    const currentIndex = filteredAndSortedImages.findIndex((img:any) => img.id === selectedImage.id)
    let newIndex
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="max-w-7xl mx-auto space-y-6">
              ...
              >
                ...
                {location !== "All" && (
                  <Badge variant="secondary" className="ml-2">
                    {data.galleryImagesCollection.items.filter((img:any) => img.location === location).length}
                  </Badge>
                )}
              </Button>
            ))}
        ...
        <div className="text-sm text-muted-foreground">
          Showing {filteredAndSortedImages.length} of {data.galleryImagesCollection.items.length} images
        </div>

        {/* Gallery Grid */}
        <div className={getGridClasses()}>
          {filteredAndSortedImages.map((image:any) => (
            <div
              key={image.id}
